...
<!--
        <div class="uk-width-medium-1-3">
            <div class="uk-grid">
                ...
                <div class="uk-width-5-6">
                    <h2 class="uk-h3">logo识别</h2>
                    <p>请上传logo图片</p>
                    <input type="file" name="logo_pic" id="logo_pic" accept="image/gif/jpg" onchange="logoPreview()" />
                    <img id="logo_pic_show" name="logo_pic_show" src="" style="width: 100px; height: 100px" />
                    <input type="button" name="uploadPicButton" value="上传" onclick="logoPic()" />
                    <div class="uk-grid" data-uk-grid-margin >
                        <div class="uk-width-medium-1-1">
                            <pre id="logoPre"><p>返回结果</p></pre>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function generalPic() {
        ...
        $.ajax({
            url:"<?php echo U('Imagedetect/generalDetectApi');?>",
            type:"post",
            // Form数据
            data: fd,
            cache: false,
    ...
    $.ajax({
        url:"<?php echo U('Imagedetect/plantDetectApi');?>",
        type:"post",
        // Form数据
        data: fd,
        cache: false,
    ...
    function animalPic() {
        ...
        $.ajax({
            url:"<?php echo U('Imagedetect/animalDetectApi');?>",
            type:"post",
            // Form数据
            data: fd,
            cache: false,
    ...
    $.ajax({
        url:"<?php echo U('Imagedetect/dishDetectApi');?>",
        type:"post",
        // Form数据
        data: fd,
        cache: false,
    ...
    function carPic() {
        ...
        $.ajax({
            url:"<?php echo U('Imagedetect/carDetectApi');?>",
            type:"post",
            // Form数据
            data: fd,
            cache: false,
    ...
    /*
    function logoPic() {
        var pic = $('#logo_pic')[0].files[0];
        var fd = new FormData();
        $('#loading').show();
        fd.append('logo_pic', pic);
        $.ajax({
            url:"<?php echo U('Imagedetect/logoDetectApi');?>",
            type:"post",
            // Form数据
            data: fd,
            cache: false,
            ...
            success:function(data){
                ...
                $('#logoPre').jsonViewer(input, options);
            }
        });
    ...
    */
    ...
    function logoPreview() {
        var r= new FileReader();
        f=document.getElementById('logo_pic').files[0];

        r.readAsDataURL(f);
        r.onload=function (e) {
            document.getElementById('logo_pic_show').src=this.result;
        };
    }*/
